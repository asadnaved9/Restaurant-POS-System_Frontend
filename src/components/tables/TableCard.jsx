...
const TableCard = ({ key, name, status, initials, seats }) => {
  ...
  return (
    <div
      ...
      key={key}
      className="w-[300px] hover:bg-[#383838] bg-[#262626] p-4 rounded-lg cursor-pointer"
    >
      <div className="flex items-center justify-between px-1">
        <h1 className="text-[#f5f5f5] text-xl font-semibold">{name}</h1>
        <p
          className={`px-2 py-1 rounded-lg ${status === "Booked" ? "bg-[#2e4a40] text-green-600" : "bg-[#664a04] text-white"}`}
        >
          {" "}
          {status}{" "}
        </p>
